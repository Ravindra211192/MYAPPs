...
{			//      5432109876543210
	...
	//##TRISB		= 0B0001111000101110;       //502
    TRISB = 0B0001111000001110;  //#5th bit 0 to be output
...
//Grosse Nummer grosse Prioritaet
	IPC14bits.PSEMIP	=	6;                      // Special Interrupt hoechste Prioritaet
    IPC18bits.PSESIP    =   7;
...
//	FCLCON1bits.CLMOD = 1;							// forces PWMxH, PWMxL pins to CLDAT values (cycle)
	FCLCON1bits.FLTSRC =13;              			// Fault13 als Ueberstrom
...
//		b0. 1 ;             kein RANGE bei EP_502
		 ...
		 //5432109876543210
CMP2CON =0B1000000000000001;			// komparator 2A an AN2 
CMP1CON =0B1000000010000001;			// komparator 1C zu AN0 
CMP3CON =0B1000000010000001;            // komparator 3C zu AN6 fuer PWM1
CMP4CON =0B0000000000000000;            // komparator 4A zu AN6 fuer PWM2
...
void Init_ADC(void)
 {
    ...
    ADCORE2Hbits.ADCS = 0b0000011; //## ADCORE2
    ...
    ADCORE2Hbits.RES = 0b11; //## Dedicated ADC Core 2 Resolution Selection bits 10 = 10bit
